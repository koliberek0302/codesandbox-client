...
const BUNDLER_URL =
  process.env.CODESANDBOX_ENV === 'development'
    ? 'http://localhost:3002'
    : `https://sandpack-${version.replace(/\./g, '-')}.csb-bogdan.dev`;
...
export default class PreviewManager {
  ...
  public getCodeSandboxURL() {
    ...
    return fetch('https://csb-bogdan.dev/api/v1/sandboxes/define?json=1', {
      method: 'POST',
      ...
      .then((res: { sandbox_id: string }) => ({
        ...
        editorUrl: `https://csb-bogdan.dev/s/${res.sandbox_id}`,
        embedUrl: `https://csb-bogdan.dev/embed/${res.sandbox_id}`,
      }));
  }
